...
import bs4
import csv
from dataDB import insertTable


class avitoParsing():
    def __init__(self, url, base_url, page_part, query_part, cur, db):
        self.InitPars(url = url, base_url=base_url, page_part=page_part, query_part=query_part, cur = cur, db = db)
...
#------------------------------------------
    def get_page_date(self, html, cur, db):
        soup = bs4.BeautifulSoup(html, 'lxml')
        ...
        # title url, price, metro
            ...
            if 'htc' in name:
                ...
                #write_csv(data)
                insertTable(cur, db , **data)

            else:
                continue
#-----------------------------------------------------------------
    def InitPars(self, url, base_url, page_part, query_part, cur, db):

        total_pages = self.get_total_pages(self.get_html(url))

        for i in range(1, total_pages):
            url_gen = base_url + page_part + str(i) + query_part
            html = self.get_html(url_gen)
            self.get_page_date(html, cur, db)
#----------------------------------------------------------------
